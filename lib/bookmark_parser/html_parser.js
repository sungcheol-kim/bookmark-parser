...
module.exports = fileStream => {
  ...
  return eachLine(fileStream, line => {
    ...
    if (line.match(/<DT>(<A.+<\/A>)<\/DT>/)) {
      ...
      let json = '';
      try {
        json = xml2json.toJson(str[1].replace(/&/g, '%26'));
      } catch (err) {
        const startIndex = str[1].indexOf('"javascript');
        let endIndex = str[1].indexOf(" ADD_DATE");
        if (endIndex === -1) {
          endIndex = str[1].indexOf(" >");
        }
        const source = str[1].replace(str[1].substring(startIndex, endIndex), '"#"');
        json = xml2json.toJson(source);
      }
